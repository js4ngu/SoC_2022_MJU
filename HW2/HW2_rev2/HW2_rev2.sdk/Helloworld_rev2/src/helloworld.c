	...
	#define LN 3
	#define LP 3
	#define BYTES_TO_TRANSFER	128*256
	#define BYTES_TO_RECEIVE	4*BYTES_TO_TRANSFER
	...
	******************************************************************************/
	...
	int main(void) {
		...
		for (int i = 0; i < LM; i++) {
			for (int j = 0; j < LN; j++) {
				if(i == j) TxBuffer[i][j] = 1;
				else  TxBuffer[i][j] = 0;
			}
		}
		for (int i = 128; i < 128+LN; i++) {
			for (int j = 0; j < LP; j++) {
				if(i == (j+128)) TxBuffer[i][j] = 1;
				else  TxBuffer[i][j] = 0;
			}
		}
		//xil_printf("=========================================================\n");
		...
		if (Status != XST_SUCCESS) {
			xil_printf("\rTX Fail\n\r");
			xil_printf("ERR CODE : %d\n\r", Status);
			return XST_FAILURE;
		}
		delay();

		Status = XAxiDma_SimpleTransfer(&AxiDma,(u32) RxBuffer, BYTES_TO_RECEIVE, XAXIDMA_DEVICE_TO_DMA);
		if (Status != XST_SUCCESS) {
			xil_printf("\rRX Fail\n\r");
			xil_printf("ERR CODE : %d\n\r", Status);
			return XST_FAILURE;
		...
		while (!TxDone && !RxDone && !Error) { /* N OP */ }
		xil_printf("\rNow finish to wait R/TX\n\r");
		...
		Xil_DCacheInvalidateRange((u32)RxBuffer, BYTES_TO_RECEIVE);
